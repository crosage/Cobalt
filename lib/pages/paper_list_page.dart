      ...
      body: RefreshIndicator(
        ...
        child: CustomScrollView(
          ...
          slivers: [
                ...
                child: Row(
                  children: [
                    Container(
                      ...
                      child: Text(
                        isRagMode ? '${_ragPapers.length} 条 RAG 结果' : '$_total 篇',
                        style: TextStyle(
                          fontSize: 12,
                    ...
                    if ((!isRagMode && _loading && _papers.isNotEmpty) ||
                        (isRagMode && _researchLoading && _ragPapers.isNotEmpty))
                      SizedBox(
                        width: 16,
                        height: 16,
            ...
            else if (isRagMode && _researchLoading && _ragPapers.isEmpty)
            ...
            // ── 论文列表 ──
            if (isRagMode && query.isNotEmpty && !_researchLoading && _ragPapers.isEmpty)
              SliverFillRemaining(
                child: _EmptyState(
                  icon: Icons.manage_search_rounded,
                  title: '暂无 RAG 结果',
                  subtitle: _researchError ?? '换一个查询试试',
                  action: TextButton.icon(
                    onPressed: _loadResearchResults,
                    icon: const Icon(Icons.refresh_rounded),
                    label: const Text('重试'),
                  ),
                ),
              )
            else if (isRagMode && _ragPapers.isNotEmpty)
              SliverPadding(
                padding: EdgeInsets.symmetric(horizontal: isCompact ? 8 : 12),
                sliver: SliverList.separated(
                  itemCount: _ragPapers.length,
            ...
            else if (!isRagMode && _loading && _papers.isEmpty)
              const SliverFillRemaining(
                child: Center(child: CircularProgressIndicator()),
              )
            else if (!isRagMode && _papers.isEmpty)
              SliverFillRemaining(
                child: _EmptyState(
